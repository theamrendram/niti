  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    setKanbanData((prev) => {
      ...
      if (fromCol === toCol) {
        ...
        toCol.cardData.splice(newIdx, 0, movedCard as kanbanCardProps);
      } else {
        // Cross-column movement
        if (toIdx === -1) {
          // Dropped into empty column or beyond last card
          toCol.cardData.push(movedCard as kanbanCardProps);
        } else {
          toCol.cardData.splice(toIdx, 0, movedCard as kanbanCardProps);
        }
      }
